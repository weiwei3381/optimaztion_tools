...
// todo 修改算法使其仅仅暴露run和getResult方法
class CLPSO {
    // 构造函数
    ...
    init() {
        ...
        for (let i = 0; i < this.particleNum; i++) {
            ...
            // 默认的范例, 所有粒子都从自身学习
            const exemplar = nc.add(nc.floor(nc.random([this.dimension])), i);
    ...
    step_run(steps) {
        ...
        for (let i = 0; i < steps; i++) {
            ...
            for (let particle of this.particleList) {
                ...
                // 获取这一步的Pbest
                const stepPbest = nc.random([this.dimension]);
                // 范例的每一个维度对应的是第i个粒子
    ...
    selectExemplar(particle) {
        const zeros = nc.floor(nc.random([this.dimension]));
        // 默认的范例, 所有粒子都从自身学习
        const exemplar = nc.add(zeros, particle.id);
        // 如果随机数大于学习率, 则从自身的pbest中学习
        const learnFromSelf = nc.gt(nc.random([this.dimension]), particle.learnProb);
        // 如果全部是从自身学习, 则随机找一个维度, 设为随机的粒子
        if (nc.all(learnFromSelf)) {
...
class Particle {
    ...
    init() {
        ...
        // 随机初始化速度,全为0-10的随机数
        this.velocity = nc.mul(nc.random([this.dimension]), 10);
        // 当前粒子的pbest就是这个位置
    ...
    get_rand_vector() {
        // 先产生一组0-1随机值
        let rand_vec = nc.random([this.dimension]);
        // 下面完成的过程是： lb + (ub - lb) * 初始值
        let range = nc.sub(this.ub, this.lb);
        rand_vec = nc.add(this.lb, nc.mul(rand_vec, range));
        return rand_vec;
    };
    ...
    update(w, stepPbest) {
        ...
        const c = 1.49445;
        // 获取速度部分
        let velocity_part = nc.mul(w, this.velocity);
        let pbest_part = nc.mul(c, nc.random([this.dimension]), nc.sub(stepPbest, this.position));
        this.velocity = nc.add(velocity_part, pbest_part);
        // 更新位置：x(t+1) = x(t) + v(t)
        this.position = nc.add(this.position, this.velocity);

        // 如果更新后的位置大于ub，则放到ub上
        let gt_parts = nc.gt(this.position, this.ub);
        ...
        // 如果更新后的位置小于lb，则放到lb上
        let lt_parts = nc.lt(this.position, this.lb);
